...
| 論点 | 決定 |
|------|------|
| ルートURL構成 | A案をルートに昇格。比較ページとB/C/D案は `archive/` へ |
| サイズ絞り込み | 実データが無いため**保留**。色・価格・シルエット・ライン・名前検索の5軸で実装 |
| モバイル縦短縮 | 2カラム + 初期6件「さらに表示」 |
| 公開範囲 | public + noindex（robots.txt + 全ページ meta robots） |

## 計画から変わった点

- **シルエット軸を追加**（当初はサイズの代替として検討）。`silhouette` は既にデータにあったが
  ウエディング全件が「Aライン」で実態と合っていなかったため、全画像を見て補正した。
  これによりサイズ抜きでもウエディングタブに3軸（色・価格・シルエット）が立った。
- **絞り込みパネルはPCも初期状態を閉じた**。開いた状態にすると商品が画面外へ押し出されるため。
  ただし条件つきURLで開かれた場合は自動展開する。
- **`srcset` を追加**（当初計画に無し）。スマホでグリッド画像が3〜4倍の過剰解像度だったため、
  440w を生成して振り分けた。拡大表示は `data-full` で原寸を読むので画質は落ちない。
- **フォトジャーニー画像を900px→560pxへ**。最大242px表示に対して過剰だった。
- **ライトボックスに `refresh()` を追加**。絞り込みで隠れた商品へ送られる不具合を防ぐため
  （読み込み時にスライド一覧を確定する実装だったため、絞り込みと相性が悪かった）。

## 計測結果

| 項目 | 変更前 | 変更後 |
|------|--------|--------|
| トップの画像合計 | 21.8MB | 3.6MB |
| スマホ初期表示の転送量 | 実測不能（画像が巨大すぎ） | **240KB** |
| スマホで全部読み込んだ後 | — | 657KB |
| コレクション章の縦（375px） | 約 4,700px | 約 1,700px |
| 絞り込み後（例: 青+緑 4件） | — | 約 900px |

## 検証

- `node tools/check-links.mjs` — 565参照すべて解決
- `node tools/audit-mobile.mjs <URL>` — 320/375/390/414/768/1024/1440px × 4ページ
  - 横スクロール: なし
  - タップ領域 44px未満: なし
  - 11px未満の文字: なし
  - JSエラー・警告: なし
  - 画像の過剰解像度: 詳細ページのメイン画像1点のみ（拡大表示用に高解像度が必要な箇所のため許容）

## 残っている検討事項

1. **サイズ絞り込み** — サイズ表をいただければ `assets/data.js` に `sizes` を足すだけで軸が増える
2. **予約フォームの実送信** — 現在はデモ。実運用には Formspree 等の送信先が必要
3. **予約フォームの「気になるドレス」select** — 手書きのままで9点のみ（データは32点）。
   `tools/build-collection.mjs` で生成するようにすれば二重管理が解消できる
4. **フォント3ファミリー** — Cormorant Garamond / Shippori Mincho / Noto Sans JP。
   CSS+フォントで約131KB。A案の意図的な組み方だが、削るなら Noto Sans JP の
   ウェイトを絞るのが効く
5. **クライアント許諾** — public リポジトリなので、松尾株式会社の企業情報・実商品画像が
   誰でも閲覧できる状態。noindex は入れているが、URLを知る人には見える

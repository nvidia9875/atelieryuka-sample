...
/**
 * ドレスのサイズ表【仮データ】
 * 2026-09-09 に先方から届いた構想図の写真（サイズ表の一部がぼやけている）から起こした値。
 * 正式な表（Excel等）を受領したら、この配列だけ差し替える。参考身長はどの行も 160cm。
 * [サイズ, 参考号数, バスト下限, バスト上限, ウエスト下限, ウエスト上限, ヒップ下限, ヒップ上限, 背丈]
 */
AY.dressSizeChart = [
  ["3FT", "3〜5号", 76, 80, 56, 60, 84, 88, 36],
  ["5FT", "5〜7号", 80, 84, 60, 64, 88, 92, 36.5],
  ["7FT", "7〜9号", 84, 88, 64, 68, 92, 96, 37],
  ["9FT", "9〜11号", 88, 92, 68, 72, 96, 100, 37.5],
  ["11FT", "11〜13号", 92, 96, 72, 76, 100, 104, 38],
  ["13FT", "13〜15号", 96, 100, 76, 80, 104, 108, 38.5],
  ["15FT", "15〜17号", 100, 104, 80, 84, 108, 112, 39],
  ["17FT", "17〜19号", 104, 108, 84, 88, 112, 116, 39.5],
  ["19FT", "19〜21号", 108, 112, 88, 92, 116, 120, 40],
];
AY.dressSizeChartStatus = "仮";
AY.dressSizeRefHeight = 160;

/**
 * ドレスのサイズ診断: バスト・ウエスト・ヒップ(cm) → 最も近いサイズ。
 * 各寸法が表の範囲に入っていれば距離0、外れていれば外れた分を距離にして、合計が最小の行を選ぶ。
 * バスト・ウエストを重く、ヒップは軽く見る（ドレスはヒップより上半身で決まるため）。
 * ヒップは任意。身長は判定に使わず、丈の参考として結果に添える。
 */
AY.suggestDressSize = function (bust, waist, hip) {
  if (!bust || !waist) return { ok: false, reason: "バストとウエストをご入力ください。" };
  if (bust < 60 || bust > 150 || waist < 45 || waist > 140 || (hip && (hip < 60 || hip > 160))) {
    return { ok: false, reason: "数値をご確認ください。" };
  }
  var gap = function (v, lo, hi) { return v < lo ? lo - v : v > hi ? v - hi : 0; };
  var best = null;
  AY.dressSizeChart.forEach(function (row) {
    var d = gap(bust, row[2], row[3]) * 1.0 + gap(waist, row[4], row[5]) * 1.0 + (hip ? gap(hip, row[6], row[7]) * 0.6 : 0);
    /* 同点（境界値）は大きい方のサイズを採る。構想図の例（バスト84・ウエスト64 → 7）と同じ挙動 */
    if (!best || d <= best.d) best = { d: d, row: row };
  });
  var r = best.row;
  var outside = best.d > 6;
  return {
    ok: true,
    size: r[0],
    go: r[1],
    spec: { bust: r[2] + "〜" + r[3], waist: r[4] + "〜" + r[5], hip: r[6] + "〜" + r[7] },
    /* 表の端から大きく外れる場合は、規格外の可能性を添える */
    caution: outside ? "ご入力の寸法はサイズ表の範囲から離れています。お直しやご相談が必要な場合がありますので、お問い合わせください。" : "",
  };
};
